...
/*************************************************************************************/
/**
 * @file      40_select_rot13_server.c
 * @brief     Exercise on select()
 * @details   This program is TCP server which returns every character rotated by
 *            13 (plus 13, modulo 26) uses select()
 * @see
...
int main() {
  ...
  if (printf("FD_SETSIZE = %d\n", FD_SETSIZE) < 0) {
    perror("printf error");
    return EXIT_FAILURE;
  }
  
  for (client_sockfd = 0; client_sockfd<FD_SETSIZE; client_sockfd++) {
    target[client_sockfd] = 0;
  }

  if ((server_sockfd = socket(AF_INET, SOCK_STREAM, PF_UNSPEC)) < 0) {
    printf("error no >> %d\n", errno);
    perror("socket");
    return EXIT_FAILURE;
  }
  ...
  FD_ZERO(&test_set);
  FD_SET(server_sockfd, &test_set);
  if (printf("Server Ready!!\n") < 0) {
    perror("printf error");
    return EXIT_FAILURE;
  }
  ...
  while (1) {
    ...
    if (FD_ISSET(server_sockfd, &ready_set)) {
      ...
      FD_SET(client_sockfd, &test_set);
      if (printf("new connection on client_sockfd %d\n", client_sockfd) < 0) {
        perror("printf error");
        return EXIT_FAILURE;
      }
      target[client_sockfd] = 1 + rand()%1000;
    }
     
    for (client_sockfd = 0; client_sockfd < FD_SETSIZE; client_sockfd++) {
      if (client_sockfd == server_sockfd) {
        continue;
      }
      if (target[client_sockfd] == 0) {
        continue;
      }
      if (FD_ISSET(client_sockfd, &ready_set)) {
